...
export class OwnQuestionComponent implements OnInit {
  ...
  onClick (target) {
    ...
    switch (this.state) {
      case FormState.Cat: return this.state = FormState.See;
      case FormState.See: return this.startTimer();
      case FormState.Think: return this.stopTimer();
      case FormState.WaitAnswer: return this.state = FormState.Answer;
      default: return this.router.navigate(['/categories']);
    }
  }
  ...
  startTimer () {
    ...
    this.timer = setInterval(() => {
      // TODO play sound
      --this.expired;
      if (this.expired < 0.5) this.stopTimer();
    }, 1000);
  }
